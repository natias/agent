...
java.util.Map<String,java.awt.font.TextLayout> cacheTLay=new java.util.HashMap<>();
    ...
    private TextLayout getNL(String s,float spanLeft, int toIteratorIndex, boolean requireNextWord) {
        ...
        int measurerPosition=measurer.getPosition();

        float c_spanLeft=spanLeft;
        int toi=toIteratorIndex;
        boolean req=requireNextWord;
        ...
        stringBuilder.append(toi);
        stringBuilder.append(requireNextWord);

        String hash=stringBuilder.toString();

        TextLayout ret=cacheTLay.get(hash);

        if(ret !=null )
            return ret;


        ret=measurer.nextLayout(spanLeft, toIteratorIndex,
                requireNextWord);

        cacheTLay.put(hash,ret);
